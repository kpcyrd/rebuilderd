...
/// Fetch repomd.xml, then download, decompress and parse the primary index.
pub async fn fetch_package_index(
    http: &http::Client,
    base_url: &str,
) -> Result<Vec<PackagesXmlItem>> {
    let bytes = fetch_url_or_path(http, &format!("{base_url}/repodata/repomd.xml")).await?;
...
pub(crate) fn parse_package_index<R: Read>(r: R) -> Result<Vec<PackagesXmlItem>> {
